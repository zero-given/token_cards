...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedFilters) {
      const { gradientColor1, gradientColor2, bgGradientColor1, bgGradientColor2 } = JSON.parse(savedFilters);
      setColors({
        gradientColor1,
        gradientColor2,
        bgGradientColor1,
  ...
  const handleColorChange = useCallback((newColors: Colors) => setColors(newColors), []);
  ...
  const performanceObserver = new PerformanceObserver((list) => {
    ...
    if (lastEntry && extendedPerformance.memory) {
      setPerformanceMetrics(prev => ({
        ...prev,
        lastMeasuredTime: lastEntry.duration,
        heapSize: extendedPerformance.memory.usedJSHeapSize / (1024 * 1024)
      }));
    } else {
      setPerformanceMetrics(prev => ({
        ...prev,
        lastMeasuredTime: lastEntry?.duration || 0,
        heapSize: prev.heapSize
      }));
    }
  });
    ...
    <Profiler id="App" onRender={onRenderCallback}>
      <ColorContext.Provider value={colorContextValue}>
        <div className="min-h-screen" style={{
          background: `linear-gradient(to right, ${colors.bgGradientColor1}, ${colors.bgGradientColor2})`
        }}>
          ...
          <div className="fixed top-0 left-0 right-0 h-28 flex flex-col justify-center w-full" style={{
            ...
            <div className="w-[90%] mx-auto backdrop-blur-md shadow-lg border border-gray-500 rounded-xl" style={{
              background: `linear-gradient(to right, ${colors.gradientColor1}E6, ${colors.gradientColor2}E6)`
            }}>
